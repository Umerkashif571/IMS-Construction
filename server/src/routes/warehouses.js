...
router.post('/', authenticate, authorize('owner', 'admin', 'store_manager'), async (req, res) => {
  try {
    const { name, type, location, city, manager_name, contact_phone, project_id } = req.body;
...
router.put('/:id', authenticate, authorize('owner', 'admin', 'store_manager'), async (req, res) => {
  try {
    const { name, type, location, city, manager_name, contact_phone, project_id, is_active } = req.body;
    const { rows } = await pool.query(
