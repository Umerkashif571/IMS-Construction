...
const NavItem = memo(function NavItem({ item, collapsed, isActive, onClick }) {
  return (
    <NavLink
      to={item.to}
      onClick={onClick}
...
const Sidebar = memo(function Sidebar({ user, collapsed, sidebarOpen, setSidebarOpen, setCollapsed, logout, navigate }) {
  ...
  const visibleItems = useMemo(() => navItems.filter(item => item.roles.includes(user?.role)), [user?.role])
...
const Header = memo(function Header({ user }) {
  ...
  return (
    <header className="bg-white border-b border-slate-200 h-16 flex items-center gap-3 px-4 lg:px-6 no-print sticky top-0 z-10">
      <button className="lg:hidden p-2 hover:bg-slate-100 rounded-lg text-slate-600" onClick={() => {}}>
        <Menu size={20} />
      </button>
      <div className="flex-1" />
      <Bell size={18} className="text-slate-400 hover:text-slate-600 cursor-pointer transition-colors" aria-label="Notifications" />
